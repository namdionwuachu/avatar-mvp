...
class AvatarMvpStack(cdk.Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        check_status_fn = lambda_python.PythonFunction(
            ...
            timeout=Duration.seconds(30),
            memory_size=256,
            environment=lambda_env,   # ONLY BUCKET_NAME, JOBS_TABLE_NAME, LOG_LEVEL
        )
        ...
        mux_fn = _lambda.DockerImageFunction(
            ...
            # For a 6s 720p video, you can safely start at 1024MB
            memory_size=1024,
            environment={
                **lambda_env,
                "FINAL_PREFIX": "renders/final/",
        ...
        definition = (
            ...
            .next(
                ...
                .when(
                    ...
                    mux_task,             # run mux when video is ready
                )
                .otherwise(fail_state)
            )
        )

        # After muxing, we’re done
